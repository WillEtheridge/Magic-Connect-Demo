...
export default function App() {
  ...
  const sendRequest = () => {
    let nftKey = (document.getElementsByName("nftKey")[0].value)
    console.log(nftKey)
    let addy = userAddress()

  }

  const userAddress = async () => {
    try {
      const [userPublicAddress] = await web3.eth.getAccounts();
      ...
      {account && (
        <>
          <form id="form" className="form">
            <label form="fname">NFT Unlock Key</label>
            <input type="text" id="nftKey" name="nftKey"></input>
          </form> 
        <button onClick ={sendRequest} className="button-row">Claim</button>
          ...
          <button onClick={userAddress} className="button-row">
